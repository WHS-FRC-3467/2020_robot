...
public class runGateAuto extends SequentialCommandGroup {
  ...
  public runGateAuto(final GateSubsystem gateSubsys, final DoubleSupplier runGate, final DoubleSupplier runGateMultiple) {
    m_runGate = runGate;
    ...
    if (m_runGate.getAsDouble() > 0.3) {
      addCommands(
        new runShooterGate(m_gateSub, 1.0).withTimeout(Constants.ShooterConstants.kShooterGateRunTime),
        new WaitCommand(2));
